...
for i in  range(0,17): #@IndentOk
   for j in range(0,17):
        tmpProduct=arr[i][j]*arr[i+1][j+1]*arr[i+2][j+2]*arr[i+3][j+3]
        if(tmpProduct>maxproduct):
            maxproduct=tmpProduct;


for i in  range(20,3): #@IndentOk
   for j in range(20,3):
        tmpProduct=arr[i][j]*arr[i-1][j-1]*arr[i-2][j-2]*arr[i-3][j-3]
        if(tmpProduct>maxproduct):
            maxproduct=tmpProduct;
